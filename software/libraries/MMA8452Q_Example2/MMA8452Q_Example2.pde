...
{  
  ...
  /* If int1 goes high, all data registers have new data */
    // Interrupt pin, should probably attach to interrupt function
  if (readRegister(0x00)&0x7 || digitalRead(int1Pin) || digitalRead(int2Pin)) // Polling, you can use this instead of the interrupt pins
  {
  ...
  //if(digitalRead(int1Pin) || digitalRead(int2Pin)){
//    source = readRegister(0x2D);  // Read the interrupt source reg.
//    if(source==0){
//  writeRegister(0x2C, 0x01);  // Active high, push-pull
  //writeRegister(0x2D, 0x18);  // DRDY int enabled, P/L enabled
//  writeRegister(0x2E, 0x10);  // P/L on INT1, pulse on INT2
      
 //   }
    Serial.print(PIND & 0x0c, HEX);Serial.print("  ");
 ...
 // if (digitalRead(int2Pin))
  {
    ...
    if ((source & 0x08)==0x08)  // If the pulse bit is set, go check those registers
      tapHandler();
  }
